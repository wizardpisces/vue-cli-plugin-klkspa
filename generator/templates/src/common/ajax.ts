...
function ajaxBase(options: AxiosRequestConfig) {
    ...
    options.headers['Accept-Language'] = getLangFromCookie().replace('-', '_');

    return service(options).then(handleResponse).catch(handleError);
}

const request = {
    get(url: string, data: object, options = {}) {
        return ajaxBase(Object.assign(options, {
            url,
            method: 'GET',
            params: data,
        }));
